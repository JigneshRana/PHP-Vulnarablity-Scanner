...
def analysis(path, plain,severity_level):
    global result_count
    global result_files
    result_files += 1
    logstr("analysis:" + path + "severity_level:"+str(severity_level))
    with open(path, 'r', encoding='utf-8', errors='replace') as content_file:
        ...
        if(int(severity_level) < 3 ):
            ...
            for credential in credz:
                ...
                for vuln_content in matches:
                    if credential in vuln_content.lower():
                        payload = ["", "Payment or Card Details Are Involved", [],"2"]
                        add_vuln_var(payload, plain, path, vuln_content, content, regex_var_detect)            
        ...
        for payload in payloads:
            ...
            if(int(payload[3]) < int(severity_level)):
                continue

            regex = re.compile(payload[0]+payload[4] + regex_indicators)
            matches = regex.findall(content.replace(" ", "(PLACEHOLDER"))
            regex_simple = re.compile(payload[0]+payload[4] + regex_indicators_simple)
            simple_matches = regex_simple.findall(content)
            ...
            for vuln_content in matches:
                ...
                if not check_protection(payload[2], vuln_content):
                    ...
                    for vulnerable_var in regex.findall(sentence):
                        ...
                        #vuln_content = simple_matches
                        #logstr(simple_matches);
                        if not false_positive:
                            result_count = result_count + 1
                            display(path, payload, vuln_content, line_vuln, declaration_text, line, vulnerable_var[1], occurence, plain,simple_matches)
  
# Run thru every files and subdirectories
def recursive(dir, progress, plain,severity_level):
    progress += 1
    progress_indicator = '⬛'
    try:
        ...
        for name in os.listdir(dir):
            ...
            if os.path.isfile(os.path.join(dir, name)):
                if ".php" in os.path.join(dir, name):
                    ...
                    analysis(dir + "/" + name,plain,severity_level)
            else:
                recursive(dir + "/" + name, progress,plain,severity_level)
   
    except OSError as e:
        print("Error 404 - Not Found, maybe you need more right ? or check help for more details." + " " * 30)
        exit(-1)

# Run thru every files and subdirectories
def multifile(str_file_list, progress, plain,severity_level):
    try:
        file_list = str_file_list.split(",")
        for file_name in file_list:
            if(file_name.split('.')[1] == "php"):
                if (os.path.isfile(file_name)):
                    analysis(file_name,plain,severity_level)
                else:
                    logstr("File not found:"+file_name)    
